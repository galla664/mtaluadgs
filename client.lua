...
function dgsSetData(element,key,value,check)
	...
	if isElement(element) and dgsType then
		...
		if dgsType == "dgs-dxscrollbar" and key == "visible" then
			dgsElementData[element].renderScrollable = value
		else
...
function GUIRender()
	local mx,my = 0, 0
	local no_cursor = not isCursorShowing()
	if no_cursor then
	...
	local bottom_table_amount = #BottomFatherTable
	local max_table_amount = #MaxFatherTable

	if bottom_table_amount + max_table_amount > 0 then
		local renderGUI = renderGUI
		for i=1, bottom_table_amount do
		...
		for i=1, max_table_amount do
			local v = MaxFatherTable[i]
			local eleData = dgsData[v]
			renderGUI(v,mx,my,{eleData.enabled,eleData.enabled},eleData.renderTarget_parent,0,0,1,eleData.visible)
		end
		dxSetRenderTarget()
		triggerEvent( "onDgsRender", localPlayer )
	end
	if no_cursor then
		MouseData.hit = false
		MouseData.Move = false
...
function renderGUI(v,mx,my,enabled,rndtgt,OffsetX,OffsetY,galpha,visible)
	...
	local selected_prioritized_element
	local eleData = dgsElementData[v]
	local enabled = {enabled[1] and eleData.enabled,eleData.enabled}
	if ( eleData.visible or eleData.renderScrollable ) and visible and isElement(v) then
		...
		if dxType == "dgs-dxwindow" then
			if x and y then
				...
				local align_horizontal,align_vertical = eleData.titalign[1], eleData.titalign[2]
				local titsidegap = eleData.titsidegap or 0
				dxDrawText(eleData.text,x+titsidegap,y,x+w-titsidegap,y+titsize,titnamecolor,txtSizX,txtSizY,font,align_horizontal,align_vertical,true,false,rendSet,eleData.colorcoded)
				------------------------------------
		...
		elseif dxType == "dgs-dxbutton" then
			...
			if x and y then
				...
				if imgs[colorimgid] then
					dxSetRenderTarget(rndtgt,nil,"add")
				...
				if #text ~= 0 then
					local font = eleData.font or systemFont
					local txtSizX,txtSizY = eleData.textsize[1],eleData.textsize[2] or eleData.textsize[1]
					local textoffset = eleData.textoffset or {0,0}
					local txtoffsetsX,txtoffsetsY = 0 + textoffset[1], 0 + textoffset[2]
					local clip = eleData.clip
					local wordbreak = eleData.wordbreak
					local colorcoded = eleData.colorcoded
					if colorimgid == 3 then
						txtoffsetsX,txtoffsetsY = eleData.clickoffset[1] + textoffset[1],eleData.clickoffset[2] + textoffset[2]
					end
		...
		elseif dxType == "dgs-dxedit" then
			...
			if x and y then
				...
				if isElement(renderTarget) then
					...
					local autocomplete_options = eleData.autocomplete_options
					local text_length = utf8.len(text)
					local text_autocomplete = ""
					if autocomplete_options and text_length > 1 then
						for i,v in ipairs(autocomplete_options) do
							if utf8.sub(v,1,text_length) == text then
								local v_length = utf8.len(v)
								if text_length ~= v_length then
									text_autocomplete = utf8.sub(v,-(v_length-text_length))
									break
								end
							end
						end
					end
					local full_text = text..text_autocomplete
					local full_text_recolored = text.."#888888"..text_autocomplete
					dgsSetData(v,"FullText",full_text)
					dxDrawText(full_text_recolored,showPos,0,w,h,textcolor,txtSizX,txtSizY,font,eleData.center and "center" or "left","center",false,false,false,true)
					dxSetRenderTarget(rndtgt)
					local finalcolor
					if not enabled[1] and not enabled[2] then
						if type(eleData.disabledColor) == "number" then
					...
					if MouseData.nowShow == v and MouseData.editCursor then
						...
						if CaretShow then
							local caretHeight = eleData.caretHeight or txtSizY*0.9
		...
		elseif dxType == "dgs-dxscrollpane" then
			...
			if x and y then
				...
				dxSetBlendMode( eleData.blend or "add")
		...
		elseif dxType == "dgs-dxscrollbar" then
			...
			if x and y then
				...
				if isMouseWithinRangeOf(x,y,w,h) then
					selected_prioritized_element = v
				end
				if MouseData.enter == v then
				...
				if voh then
					...
					if imgs[2] then
						dxDrawImage(x,y+arrowPos+pos*0.01*csRange,w,cursorRange,imgs[2],0,0,0,colors[colorimgid[2]][2],rendSet)
					else
								...
								if isDraw1 then
									...
									if sortColumn == id and sortIcon then
										if DataTab.columnShadow then
											dxDrawText(sortIcon,_tempStartx+1-10+5,1,_tempEndx,columnHeight,black,columntextx,columntexty,columnFont,"left","center",false,false,false,false,true)
										end
										dxDrawText(sortIcon,_tempStartx-10+5,0,_tempEndx,columnHeight,columnTextColor,columntextx,columntexty,columnFont,"left","center",false,false,false,false,true)
									end
									if DataTab.columnShadow then
										dxDrawText(data[1],_tempStartx+15+1,1,_tempEndx,columnHeight,black,columntextx,columntexty,columnFont,"left","center",false,false,false,false,true)
									end
									dxDrawText(data[1],_tempStartx+15,0,_tempEndx,columnHeight,columnTextColor,columntextx,columntexty,columnFont,"left","center",false,false,false,false,true)
								end
							...
							if isDraw2 then
								...
								for id,v in pairs(cpos) do
									...
									if text then
										...
										lc_rowData[id][2] = columnTextColor
										textBuffer[id] = {lc_rowData[id][1],_x-_x%1,_x-_x%1+backgroundLength,lc_rowData[id][2],_txtScalex,_txtScaley,_txtFont,clip,colorcoded}
									end
								end
	...
	if selected_prioritized_element then
		MouseData.hit = selected_prioritized_element
		MouseData.enter = selected_prioritized_element
	end
end
...
function checkEditCursor(button,state)
	if button == "mouse_wheel_up" or button == "mouse_wheel_down" then
		...
		elseif isElement(MouseData.scrollPane) then
			...
			local sbr
			if dgsGetData( scrollbar1, "renderScrollable") or dgsGetVisible(scrollbar1) then
				sbr = scrollbar1
			end
			if dgsGetData( scrollbar2, "renderScrollable") or dgsGetVisible(scrollbar2) then
				sbr = scrollbar2
			end
			if sbr then
	...
	if guitype == "gui-edit" then
		...
		if isElement(myedit) then
			if source == dgsElementData[myedit].edit then
				...
				local whiteListText = utf8.gsub(text_new,dgsElementData[myedit].whiteList or "","")
...
addEventHandler("onClientElementDestroy",resourceRoot,function()
	...
	if dgsIsDxElement(source) then
		...
		if dgsGetType(source) == "dgs-dxedit" then
			...
			if isElement( edit ) then destroyElement( edit ) end
...
addEventHandler("onClientClick",root,function(button,state,x,y)
	...
	if isElement(guiele) then
		...
		if gtype == "dgs-dximage" then return end
		if state == "down" then
			if button == "left" then
				local gtype = dgsGetType(guiele)
