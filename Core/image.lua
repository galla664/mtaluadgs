function dgsCreateImage(x,y,sx,sy,img,relative,parent,color)
	...
	if x == 0 and y == 0 and sx == screen_x and sy == screen_y then
	--local ib_bg = call( getResourceFromName( "nrp_ib" ), "ibArea", { px = 0, py = 0, sx = screen_x, sy = screen_y,
	--} )
		--call( getResourceFromName( "nrp_ib" ), "ibSetData", ib_bg, "priority", 1 )
		addEventHandler( "onClientElementDestroy", image, function()
			destroyElement( ib_bg )
		end )
	end
